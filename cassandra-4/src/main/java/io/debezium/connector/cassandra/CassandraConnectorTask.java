    ...
    static CassandraConnectorTaskTemplate init(CassandraConnectorConfig config, ComponentFactory factory) {
        return new CassandraConnectorTaskTemplate(config,
                ...
                context -> {
                    ...
                    CassandraStreamingMetrics metrics1 = new CassandraStreamingMetrics((CdcSourceTaskContext) context);

                    return new AbstractProcessor[]{ new CommitLogIdxProcessor(context, metrics1, legacyMetrics,
                            new Cassandra4CommitLogSegmentReader(context, metrics1, legacyMetrics),
