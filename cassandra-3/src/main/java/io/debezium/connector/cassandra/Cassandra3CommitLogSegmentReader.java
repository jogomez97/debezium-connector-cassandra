...
public class Cassandra3CommitLogSegmentReader implements CommitLogSegmentReader {

    private final CommitLogReader commitLogReader;

    private final CommitLogReadHandler commitLogReadHandler;

    public Cassandra3CommitLogSegmentReader(CassandraConnectorContext context, CassandraStreamingMetrics streamingMetrics, CommitLogProcessorMetrics legacyMetrics) {
        this.commitLogReader = new CommitLogReader();
        this.commitLogReadHandler = new Cassandra3CommitLogReadHandlerImpl(context, streamingMetrics, legacyMetrics);
    }
